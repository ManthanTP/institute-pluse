...
import { ShoppingCart, Plus, Minus, X, Leaf, Search, UtensilsCrossed, ChevronLeft, Clock, CheckCircle2, Timer, CreditCard } from 'lucide-react'
...
export default function FacultyCafeteriaPage() {
  ...
  useEffect(() => {
    ...
    if (profile?.id) {
      ...
      // Realtime subscription for the faculty's orders
      const channel = supabase
        .channel('faculty_orders_changes')
        .on('postgres_changes', { 
          event: 'UPDATE', 
          schema: 'public', 
          table: 'orders', 
  ...
  const handleOrder = async () => {
    ...
    const orderData = {
      student_id: profile.id, // Reusing column for simplicity (corresponds to user profile id)
      items: cart,
      total_price: total,
    ...
    <FacultyLayout>
      <div className="space-y-6 max-w-7xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <h2 className="text-xl lg:text-2xl font-black text-white uppercase tracking-tighter">Cafeteria</h2>
            <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mt-1">Order food & track carbon offset</p>
          </div>
          <div>
            <motion.button
              whileTap={{ scale: 0.9 }}
              onClick={() => setShowCart(true)}
              className="relative w-12 h-12 rounded-2xl bg-blue-600 text-white flex items-center justify-center shadow-[0_0_20px_rgba(59,130,246,0.3)] hover:bg-blue-500 transition-all"
            >
        ...
        <div className="flex gap-2 p-1 bg-white/5 border border-white/10 rounded-2xl mb-6">
          <button
            ...
            className={`flex-1 py-3 rounded-xl text-[9px] md:text-[10px] font-black uppercase tracking-widest transition-all ${
        ...
        {view === 'menu' ? (
          <>
            ...
            <div className="relative mb-6">
              ...
              <input 
                ...
                className="w-full bg-[#161b22] border border-white/5 rounded-2xl py-4 pl-12 pr-4 text-xs font-black text-white uppercase tracking-[0.2em] outline-none shadow-inner"
              />
            </div>
            ...
            <div className="flex gap-2 overflow-x-auto no-scrollbar mb-6 pb-2">
              {CATEGORIES.map(cat => (
                <button
                  ...
                  className={`px-5 py-2.5 rounded-full text-[9px] md:text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap border ${
                    activeCategory === cat 
                    ...
                    <div className="flex justify-between items-start mb-4">
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          <span className="px-2.5 py-0.5 rounded bg-blue-500/10 border border-blue-500/20 text-[8px] font-black text-blue-500 uppercase tracking-widest">
                            {item.category}
                          </span>
                          {item.is_vegetarian && (
          ...
          <div className="space-y-4">
            {orders.length === 0 ? (
              <div className="py-20 text-center bg-white/5 border border-white/10 rounded-2xl backdrop-blur-xl">
              ...
              <motion.div
                ...
                className="bg-[#161b22]/80 border border-white/5 rounded-2xl p-5 backdrop-blur-2xl flex items-center justify-between group cursor-pointer hover:bg-white/5 transition-all"
              >
                <div className="flex items-center gap-4">
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center border shrink-0 ${
                    ...
                    {order.status === 'delivered' ? <CheckCircle2 size={20} /> : <Timer size={20} className="animate-pulse" />}
                  </div>
                  <div>
          ...
          {showCart && (
            <div className="fixed inset-0 z-[9999] flex items-end justify-center pointer-events-none">
              ...
              >
                ...
                <div className="flex items-center justify-between mb-8">
                   <div className="flex items-center gap-4">
                      <div className="w-12 h-12 rounded-[20px] bg-blue-600/10 border border-blue-500/20 flex items-center justify-center text-blue-500">
                         <ShoppingCart size={22} />
                      </div>
                      <div>
                         <p className="text-[9px] font-black text-gray-500 uppercase tracking-widest mb-0.5">Payload Summary</p>
                         <h2 className="text-lg md:text-xl font-black text-white uppercase tracking-tight leading-tight">Nutrition Cart</h2>
                      </div>
                   </div>
                ...
                <div className="flex-1 overflow-y-auto no-scrollbar max-h-[40vh] space-y-3 mb-8">
                   {cart.length === 0 ? (
                ...
                <div className="space-y-6 pt-6 border-t border-white/10">
                   ...
                   <motion.button 
                    ...
                    className="w-full py-6 rounded-[28px] bg-blue-600 text-white font-black text-[10px] md:text-xs uppercase tracking-[0.4em] shadow-[0_15px_40px_rgba(59,130,246,0.4)] disabled:opacity-20 hover:bg-blue-500"
                   >
        ...
        <AnimatePresence>
          {(orderSuccess || selectedOrder) && (
            <div className="fixed inset-0 z-[10000] flex items-center justify-center p-6">
               ...
               >
                  <div className="w-20 h-20 bg-blue-600/10 border border-blue-500/20 rounded-[28px] flex items-center justify-center text-blue-500 mx-auto mb-6">
                  ...
                  <div className="mb-8 px-2">
                    <div className="flex justify-between text-[8px] font-black uppercase tracking-widest text-gray-500 mb-3">
                      ...
                      <span className="text-blue-500">{(orderSuccess || selectedOrder).status.toUpperCase()}</span>
                    </div>
                  ...
                  <div className="mb-8 text-left space-y-2">
                     ...
                     <div className="pt-2 flex justify-between text-[10px] font-black text-green-500 uppercase tracking-widest italic">
                        ...
                        <span>{(orderSuccess || selectedOrder).total_carbon_kg || 0}kg CO₂</span>
                     </div>
                  </div>
