      ...
      <div className="space-y-8 lg:space-y-12 pb-20">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 lg:gap-8">
          ...
          <div className="flex bg-[#161b22] border border-white/10 rounded-2xl p-1">
             <button onClick={() => setActiveTab('tracking')} className={`px-6 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'tracking' ? 'bg-blue-600 text-white' : 'text-gray-500 hover:text-white'}`}>Live Tracking</button>
             <button onClick={() => setActiveTab('metadata')} className={`px-6 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'metadata' ? 'bg-purple-600 text-white' : 'text-gray-500 hover:text-white'}`}>Metadata Config</button>
          </div>
        </div>
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-6">
               <div className="bg-[#161b22] border border-white/5 rounded-3xl p-8 relative overflow-hidden group">
                  ...
                  <p className="text-[9px] font-black text-blue-500 uppercase tracking-widest mb-2">Live Terminals</p>
               ...
               <div className="sm:col-span-2 lg:col-span-1 bg-[#161b22] border border-white/5 rounded-3xl p-8 relative overflow-hidden group">
                  ...
                  <p className="text-[9px] font-black text-purple-500 uppercase tracking-widest mb-2">Registry Volume</p>
            ...
            <div className="flex flex-col lg:flex-row gap-4 items-center">
                <div className="relative w-full lg:flex-1 group">
                   ...
                   <input type="text" placeholder="Search by subject or faculty..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full bg-[#161b22] border border-white/10 rounded-2xl py-5 pl-16 pr-8 text-[11px] font-black uppercase tracking-widest text-white focus:outline-none focus:border-blue-500/50" />
                </div>
                <div className="flex w-full lg:w-auto gap-1 p-1.5 bg-[#161b22] border border-white/10 rounded-2xl">
                   {['all', 'active', 'locked'].map((type) => (
                     <button key={type} onClick={() => setFilterType(type)} className={`flex-1 lg:flex-none px-8 py-3 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${filterType === type ? 'bg-blue-600 text-white' : 'text-gray-500 hover:text-white'}`}>{type}</button>
                   ))}
                </div>
            </div>
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
            <div className="flex gap-2 p-1.5 bg-[#161b22] border border-white/10 rounded-2xl w-max">
               {[
                 ...
                 <button key={t.id} onClick={() => setMetaType(t.id)} className={`px-6 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${metaType === t.id ? 'bg-purple-600 text-white' : 'text-gray-500 hover:text-white'}`}>{t.label}</button>
               ))}
            </div>
               ...
               <div className="lg:col-span-1 bg-[#161b22]/80 border border-white/5 rounded-3xl p-6">
                 ...
                 <form onSubmit={handleAddMetadata} className="space-y-4">
                   <div>
                     ...
                     <input type="text" required value={newItem.name || ''} onChange={e => setNewItem({...newItem, name: e.target.value})} className="w-full mt-2 bg-white/5 border border-white/10 rounded-xl py-3 px-4 text-sm text-white focus:border-purple-500/50 outline-none" placeholder="e.g. 1, A, Physics" />
                   </div>

                   {metaType === 'subjects' && (
                     <div>
                       <label className="text-[9px] font-black text-gray-500 uppercase tracking-widest">Subject Code</label>
                       <input type="text" required value={newItem.code || ''} onChange={e => setNewItem({...newItem, code: e.target.value})} className="w-full mt-2 bg-white/5 border border-white/10 rounded-xl py-3 px-4 text-sm text-white focus:border-purple-500/50 outline-none" placeholder="e.g. CS101" />
                     </div>
                   )}
                   ...
                   <button type="submit" className="w-full py-4 mt-4 bg-purple-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-purple-500 transition-all flex items-center justify-center gap-2">
            ...
            <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 lg:p-0">
              ...
              <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="relative w-full max-w-4xl bg-[#161b22] border border-white/10 rounded-[32px] p-6 lg:p-10 shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
                ...
                <div className="flex-1 overflow-y-auto no-scrollbar bg-black/20 rounded-2xl p-4 border border-white/5">
                   {loadingParticipants ? (
                     <div className="py-20 text-center"><RefreshCw className="mx-auto animate-spin text-blue-500 mb-4" />Loading list...</div>
