      ...
      <div className="space-y-8 lg:space-y-10 pb-20">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 lg:gap-8">
          <div className="text-center md:text-left">
            <div className="flex items-center justify-center md:justify-start gap-2 mb-2 lg:mb-3">
              ...
              <span className="text-[8px] lg:text-[10px] font-black text-blue-500 uppercase tracking-[0.3em]">Master Event Registry</span>
            </div>
            <h2 className="text-3xl lg:text-4xl font-black text-white tracking-tighter uppercase leading-none italic">Campaign <span className="text-blue-500">Control</span></h2>
          ...
          <div className="flex flex-col sm:flex-row gap-4 w-full md:w-auto">
             <div className="flex bg-white/5 border border-white/10 p-1 rounded-xl lg:rounded-2xl backdrop-blur-xl overflow-x-auto no-scrollbar">
               {['upcoming', 'completed', 'cancelled'].map(f => (
                 <button 
                   key={f}
                   onClick={() => setFilter(f)}
                   className={`flex-1 md:flex-none px-4 lg:px-6 py-2.5 rounded-lg lg:rounded-xl text-[7px] lg:text-[9px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${filter === f ? 'bg-white text-slate-950 shadow-xl' : 'text-gray-500 hover:text-white'}`}
                 >
                   {f}
                 </button>
               ))}
             ...
             <button 
               ...
               className="w-full md:w-auto px-6 lg:px-8 py-4 rounded-xl lg:rounded-2xl bg-blue-600 text-white text-[9px] lg:text-[10px] font-black uppercase tracking-[0.2em] hover:bg-blue-500 shadow-xl shadow-blue-600/20 transition-all flex items-center justify-center gap-3"
             >
        ...
        <div className="flex flex-col md:flex-row gap-6">
           <div className="flex-1 relative group">
              <Search size={16} className="absolute left-6 top-1/2 -translate-y-1/2 text-gray-500 group-focus-within:text-blue-500 transition-colors" />
              <input 
                value={search}
                onChange={e => setSearch(e.target.value)}
                placeholder="SEARCH CAMPAIGN REGISTRY..."
                className="w-full bg-white/5 border border-white/10 rounded-xl lg:rounded-[32px] py-4 lg:py-5 pl-14 lg:pl-16 pr-6 text-[9px] lg:text-[11px] font-black uppercase tracking-widest text-white outline-none focus:border-blue-500/50 transition-all"
              />
           </div>
        </div>
                ...
                >
                  <div className="flex items-start justify-between mb-6 lg:mb-8">
                     <div className="flex items-center gap-4">
                        ...
                        <div>
                           <p className="text-[7px] lg:text-[9px] font-black text-gray-500 uppercase tracking-[0.2em] mb-1">{event.category}</p>
                     ...
                     <div className="flex gap-2">
                        <button 
                          ...
                          className="p-2.5 lg:p-3 rounded-lg lg:rounded-xl bg-white/5 border border-white/10 text-gray-400 hover:text-white transition-colors"
                        >
                  ...
                  <div className="grid grid-cols-2 gap-4 lg:gap-6 mb-6 lg:mb-8">
                     <div className="space-y-3 lg:space-y-4">
                        <div className="flex items-center gap-2 lg:gap-3 text-gray-400">
                           <Calendar size={12} lg:size={14} className="text-blue-500" />
                        ...
                        <div className="flex items-center gap-2 lg:gap-3 text-gray-400">
                           <MapPin size={12} lg:size={14} className="text-blue-500" />
                     ...
                     <div className="space-y-3 lg:space-y-4">
                        <div className="flex items-center gap-2 lg:gap-3 text-gray-400">
                           <Clock size={12} lg:size={14} className="text-blue-500" />
                  ...
                  <div className="pt-6 border-t border-white/5 flex items-center justify-between">
                     ...
                     <button 
                       ...
                       className="text-[8px] lg:text-[9px] font-black text-red-500 uppercase tracking-widest hover:text-red-400 transition-colors"
                     >
                        Purge Registry
                     </button>
                  </div>
                ...
                <form onSubmit={handleSaveEvent} className="space-y-4 lg:space-y-6 overflow-y-auto no-scrollbar pr-1 pb-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 lg:gap-6">
                    <div className="space-y-2 sm:col-span-2">
                      ...
                      <input name="title" placeholder="ENTER PROTOCOL NAME..." defaultValue={selectedEvent?.title} className="w-full bg-white/5 border border-white/10 rounded-xl lg:rounded-2xl p-4 lg:p-5 text-xs lg:text-sm text-white outline-none focus:border-blue-500/30 shadow-inner" required />
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <input name="banner_color" type="color" defaultValue={selectedEvent?.banner_color || '#16a34a'} className="w-full h-[52px] lg:h-[58px] bg-white/5 border border-white/10 rounded-xl lg:rounded-2xl p-2 cursor-pointer shadow-inner" />
                    </div>
                    <div className="space-y-2">
                  ...
                  <button type="submit" className="w-full py-5 lg:py-6 rounded-[24px] lg:rounded-[32px] bg-blue-600 text-white font-black text-[9px] lg:text-[11px] uppercase tracking-[0.3em] shadow-2xl shadow-blue-600/30 hover:bg-blue-500 active:scale-95 transition-all">
                  ...
                  {participants.length === 0 ? (
                    ...
                    <div key={p.id} className="bg-white/5 border border-white/5 rounded-2xl lg:rounded-3xl p-5 lg:p-6 flex items-center justify-between hover:bg-white/[0.08] transition-all">
                      <div className="flex items-center gap-3 lg:gap-4">
                        <div className="w-8 h-8 lg:w-10 lg:h-10 rounded-lg lg:rounded-xl bg-blue-500/10 flex items-center justify-center text-[10px] lg:text-[11px] font-black text-blue-500 uppercase border border-blue-500/10">
