...
export default function AdminNotificationsPage() {
  ...
  useEffect(() => {
    // Mock Data
    setNotifications([
      { id: 1, type: 'security', title: 'Suspicious Login Attempt', content: 'Multiple failed login attempts detected from IP: 192.168.1.45 (Unauthorized Sector).', time: '5 mins ago', read: false },
      { id: 2, type: 'resource', title: 'Cafeteria Inventory Alert', content: 'Main Grain Stock is below 15%. Automated re-order protocol suggested.', time: '2 hours ago', read: false },
      { id: 3, type: 'system', title: 'Server Node Migration', content: 'Global database node has been successfully migrated to high-availability sector.', time: '1 day ago', read: true },
    ])
    setLoading(false)
  }, [])
    ...
    <AdminLayout>
      <div className="space-y-12 pb-20 max-w-5xl mx-auto">
        ...
        <div className="pt-12 border-t border-white/5">
           <div className="bg-[#0f172a]/40 border border-white/10 rounded-[60px] p-16 flex flex-col md:flex-row items-center justify-between gap-12">
              <div className="space-y-4 text-center md:text-left">
                 <h4 className="text-2xl font-black text-white uppercase tracking-tight">Notification Topology</h4>
                 <p className="text-sm text-gray-500 font-medium leading-relaxed max-w-md uppercase tracking-wide">Configure global alert relays, threshold triggers, and automated system reports for the entire campus infrastructure.</p>
              </div>
              <button className="px-12 py-6 bg-white text-slate-900 rounded-[24px] text-[10px] font-black uppercase tracking-widest shadow-2xl hover:scale-105 transition-all">Configure Nexus</button>
           </div>
        </div>
      </div>
    </AdminLayout>
