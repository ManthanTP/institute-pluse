...
import { MapPin, Search, Navigation, Clock, Star, X, ChevronLeft, Map, Compass, Building2, Wind, Home, LayoutGrid, CalendarDays, Coffee, User, ChevronRight } from 'lucide-react'
import { motion, AnimatePresence } from 'framer-motion'
import { useNavigate } from 'react-router-dom'

const LOCATIONS = [
  { id: 1, name: 'Main Auditorium', block: 'Block A', type: 'Venue', distance: '120m', floor: 'G' },
  { id: 2, name: 'Central Library', block: 'Block C', type: 'Study', distance: '450m', floor: '1-3' },
  { id: 3, name: 'Cyber Lab 04', block: 'Block B', type: 'Lab', distance: '280m', floor: '2' },
  { id: 4, name: 'Eco Garden', block: 'Nexus Hub', type: 'Recreation', distance: '50m', floor: 'G' },
  { id: 5, name: 'Cafeteria Hub', block: 'Block D', type: 'Dining', distance: '600m', floor: 'G' },
  { id: 6, name: 'Gymnasium', block: 'Block E', type: 'Sports', distance: '850m', floor: 'G' },
]
    ...
    <div className="min-h-[100dvh] bg-[#0a0c10] text-white pb-32 relative overflow-x-hidden">
      ...
      <div className="relative z-10 px-6 pt-8">
        ...
        <div className="flex gap-3 overflow-x-auto no-scrollbar mb-12 pb-2">
          {['All', 'Venue', 'Study', 'Lab', 'Dining'].map(f => (
            <button
              key={f}
              onClick={() => setFilter(f)}
            ...
            >
              <div className="flex justify-between items-start mb-6 relative z-10">
                 <div className="space-y-1">
                    ...
                    <h3 className="text-2xl font-black text-white uppercase tracking-tight leading-none">{loc.name}</h3>
                 </div>
                 <div className="text-right">
                    <p className="text-2xl font-black text-white tracking-tighter leading-none">{loc.distance}</p>
                    <p className="text-[8px] font-black text-gray-500 uppercase tracking-widest mt-1">Range</p>
                 </div>
              </div>
              ...
              <div className="flex items-center justify-between relative z-10 pt-6 border-t border-white/5">
                 <div className="flex items-center gap-6">
                    <div className="flex items-center gap-2">
                       ...
                       <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Floor {loc.floor}</span>
                    </div>
                    <div className="flex items-center gap-2">
                       <Wind size={14} className="text-green-500" />
                       <span className="text-[10px] font-black text-green-500 uppercase tracking-widest">Optimized Route</span>
                    </div>
                 </div>
                 <motion.button 
                   whileTap={{ scale: 0.9 }}
                   className="w-12 h-12 rounded-2xl bg-blue-600 text-white flex items-center justify-center shadow-[0_10px_20px_rgba(37,99,235,0.3)]"
                 >
