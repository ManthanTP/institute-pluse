...
export default function BusTrackingPage() {
  ...
  return (
    <main className="max-w-7xl mx-auto">
      {/* BUS TABS */}
      <div className="flex gap-3 overflow-x-auto no-scrollbar py-4 relative z-10 mb-2">
        {busNumbers.map(b => (
          <button 
            key={b} 
      ...
      <motion.div 
        ...
        className="relative h-72 lg:h-96 rounded-[40px] bg-slate-900 border border-white/5 overflow-hidden mb-8 shadow-2xl"
      >
      ...
      {/* TIP CARD */}
      <div className="mt-10 p-8 rounded-[40px] bg-green-600/5 border border-green-500/10 relative overflow-hidden">
        <div className="absolute top-0 right-0 p-4 opacity-5">
          <Sparkles size={100} className="text-green-500" />
        </div>
        <p className="text-[10px] font-black text-green-500 uppercase tracking-[0.2em] mb-4 flex items-center gap-2">
          <Leaf size={12} /> Planet Insight
        </p>
        <p className="text-sm text-gray-400 leading-relaxed font-medium max-w-2xl">
          Each bus commute prevents approximately 0.36kg of CO2 emissions compared to private transit. Multiply this by thousands of daily students and the impact is revolutionary.
        </p>
      </div>
    </main>
  )
